...
例:
- 標準: `.../buyout-prospects/<slug>/e-taisei/`（E案のみ）
- 旧在庫（明示指定時のみ）: `f-sanyu` / `b-atelier` / `c-daylight` / `c-refresh` / `d-signboard`
...
7. CSV の `demo_url_a` に https を記入（`skin_pair=e-taisei`。`demo_url_b` は空でよい） → `status=built` → 送信後 `sent`
