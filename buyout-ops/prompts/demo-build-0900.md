   ...
   - ★第三者チェック（作った本人の自己採点だけで queued にしない。
     ...
     1桁脱字を見逃し、独立した二次チェックで発見・修正した）:
       ...
       - Claude Code から実行しているときは Agent ツールで
         ...
         （建てたエージェント自身に検証させない）
       - モデルは model: opus を指定する（2026-09-10 オーナー方針:
         費用対効果のため業務に応じて機種を振る。事故に直結する照合系は
         Opus、G1 URL 確認など機械的な作業は Sonnet のままでよい。
         Fable は高性能との裏付けが無いため現時点では使わない）
...
| 日付 | 変更 |
|---|---|
| 2026-09-10 | 第三者チェックのサブエージェントに `model: opus` を指定（費用対効果を考えた機種振り分け方針。機械的なG1確認等はSonnetのまま） |
