...
6. デモ未公開なら `swap-prospect.mjs`（`--skins e-taisei`・先方画像禁止）→ `verify-demo-content.mjs` PASS＋FACT照合 → `publish-prospect.mjs` → **buyout-prospects を main へ直接 push（Draft PR 禁止）**
