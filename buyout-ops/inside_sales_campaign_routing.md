...
### 初回メールの構成（2026-09-01 改訂）

「頼みたい」と思わせるため、観察のあとに3ブロックを必ず入れる。具体策は `inside-fix-ideas.mjs` が `campaign_evidence` から生成。

| ブロック | 内容 |
|---|---|
| 観察 | サイトの事実ベースの指摘（既存） |
| **■ 御社の場合、こう整えるイメージ** | 観察1つにつき具体策1行（最大3） |
| **■ 進め方** | ①15分ヒアリング → ②御社向け改善イメージ1つ送付 → ③合意範囲だけ着手 |
| **信頼の一言** | 全面作り直しを勧めない・工務店向けに調査済み |
