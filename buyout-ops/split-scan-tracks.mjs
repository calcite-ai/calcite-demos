...
function main() {
  ...
  const insidePool = [];
  const reviewRows = [];
  const stats = { buyout: 0, inside: 0, inside_skip: 0, skip: 0 };
  ...
  for (const row of scanRows) {
    ...
    const existingLead = leadState.get(`${row.company}\t${row.email}`);
    ...
    const leadRow = {
      ...
      status: existingLead?.status || (rec === "skip" ? "pool_skip" : "pool"),
  ...
  fs.writeFileSync(
    ...
    serializeCsv(LEAD_HEADERS, insidePool, {
      alwaysQuoteHeaders: ["site_url", "email", "campaign_evidence"],
    }) + "\n"
  );
