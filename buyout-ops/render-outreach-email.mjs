...
const DEFECT_LINE = {
  SSL未整備: "サイトが https 未対応のままで、ブラウザによっては警告が出ます",
  "https未整備": "サイトが https 未対応のままで、ブラウザによっては警告が出ます",
  viewportなし: "スマホ表示用のviewport設定がなく、画面幅に合わせた表示になっていません",
  "tel:なし": "tel:リンクがないため、スマホで電話番号をタップしても発信につながりません",
};
...
function issueLine(item, i) {
  ...
  let text = key ? DEFECT_LINE[key] : item;
  if (/更新停止感/.test(item)) {
    text = "更新が止まっており、稼働中かどうか判断しづらい状態です";
  }
  return `${["①", "②", "③"][i] || `${i + 1}.`} ${text}`;
}
