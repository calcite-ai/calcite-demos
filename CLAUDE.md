...
**対象は先方向けデモHTML本文のみ**（`works/**` `buyout-template/**` `buyout-prospects/**`。
`verify-demo-content.mjs`/`verify-ops-pack.mjs`の実装どおり）。
営業メール本文（`templates/email_demo_buyout_*.txt`）は対象外 — 「デモサイト買い取り
プラン」が商品名のため「デモ」は必須語として意図的に使われている。
2026-09-11、独立レビューでこの誤読が繰り返し発生したため明記（本文自体の変更はなし）。
